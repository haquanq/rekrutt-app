...
class InterviewParticipantPolicy
{
    ...
    {
        if (!$user->hasRole(UserRole::HIRING_MANAGER, UserRole::RECRUITER)) {
            return Response::deny("You are not allowed to update any interview participant");
    ...
    {
        if (!$user->hasRole(UserRole::HIRING_MANAGER, UserRole::RECRUITER)) {
            return Response::deny("You are not allowed to delete any interview participant");
        } elseif (!$interviewParticipant->interview->isCreatedBy($user)) {
            return Response::deny("You are not the creator of this interview");
        }
